...
Для каждого товара обрабатываются все доступные размеры, и в выходных данных
появляются отдельные строки с полями `sizeID`, `price`, `discountedPrice` и
`clubDiscountedPrice` вместе с соответствующими скидками.
