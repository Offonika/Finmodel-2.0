...
5. Запускайте нужный скрипт через единый CLI:
   ...
   перед загрузкой новых данных. Токен можно передать через `--api-key` или
   опустить — тогда скрипт возьмёт все токены из базы `finmodel.db` и обработает
   каждую организацию по очереди:
...
из CSV, TXT или SQLite, либо не указывать список — тогда для каждой
организации из базы `finmodel.db` будет использован её собственный список
из таблицы `katalog`. Для каждого товара сохраняются цены по всем размерам
(`sizeID`). Перед записью таблица очищается. Токен можно указать вручную
через `--api-key` или загрузить автоматически из таблицы `НастройкиОрганизаций`.
...
# автоматический режим по всем организациям
python -m finmodel.scripts.wb_goods_prices_import_flat \
