...
def main():
    ...
    ap.add_argument("nmids", nargs="+", help="список nmId (через пробел) или путь к .txt/.csv c колонкой nmId")
    ...
    if args.out:
        keys=rows[0].keys() if rows else ["nmId","priceU","salePriceU","sale(%)","price_rub","salePrice_rub"]
