def main():
    ...
    def make_http(timeout: int, retries: int) -> requests.Session:
        ...
        s.headers.update(
            {"Accept": "application/json", "User-Agent": "WB-SPP-Fetcher/Final (+Finmodel2.0)"}
        )
    ...
    def _request_cards(
        http: requests.Session, endpoint: str, params: dict, headers: dict, timeout: int
    ) -> dict:
    ...
    def fetch_batch(
        http: requests.Session, ids: List[str], caps: Dict[str, Any], timeout: int
    ) -> List[Dict[str, Any]]:
    ...
    def calc_metrics(row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            **row,
            ...
            "discount_total_pct": (
                round(discount_total_pct, 2) if isinstance(discount_total_pct, float) else None
            ),
            "spp_pct_approx": (
                round(spp_pct_approx, 2) if isinstance(spp_pct_approx, float) else None
            ),
    ...
    def main():
        ...
        for i in range(total_batches):
            ...
            try:
                ...
                logging.info(
                    "Батч %d/%d: запрошено=%d → получено=%d",
                    i + 1,
                    total_batches,
                    len(part),
                    len(rows),
        ...
        if missing:
            ...
            logging.warning(
                "WB не вернул %d nmId. Примеры: %s%s",
                len(missing),
                ", ".join(missing[:10]),
